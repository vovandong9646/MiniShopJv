    ...
    <div id="content">
            ...
            <table class="shop_table beta-shopping-cart-table" cellspacing="0">
                ...
                <c:forEach items="${cart}" var="giohang">
                    ...
                    <td class="product-name">
                        <div class="media">
                            <img class="pull-left" style="width:100px;" src="<c:url value='/resources/image/product/${giohang.getProductEntity().getImage()}'/>" alt="">
                            <div class="media-body">
                                <p class="font-large table-title">${giohang.getProductEntity().getName()}</p>
                    ...
                    <td class="product-price">
                        <c:choose>
                            <c:when test="${giohang.getProductEntity().getPromotionPrice() != 0}" >
                                <span class="amount">$ ${giohang.getProductEntity().getPromotionPrice()}</span>
                            </c:when>
                            <c:otherwise>
                                <span class="amount">$ ${giohang.getProductEntity().getUnitPrice()}</span>
                            </c:otherwise>
                        </c:choose>
                    ...
                    <td class="product-status">
                        ${giohang.getProductEntity().getUnit()}
                    </td>

                    <td class="product-quantity">
                        <select name="product-qty" class="productQuantity" data-id="${giohang.getProductEntity().getId()}">

                            <c:forEach begin="1" end="20" step="1" var="stt">
                                <c:choose>
                                    <c:when test="${giohang.getQuantity() == stt}">
                                        <option selected value="${giohang.getQuantity()}">${giohang.getQuantity()}</option>
                                    </c:when>
                                    <c:otherwise>
                    ...
                    <td class="product-subtotal">
                        ...
                        <c:choose>
                            <c:when test="${giohang.getProductEntity().getPromotionPrice() != 0}" >
                                <span class="amount total">$ ${price = giohang.getQuantity() * giohang.getProductEntity().getPromotionPrice()}</span>
                            </c:when>
                            <c:otherwise>
                                <span class="amount total">$ ${price = giohang.getQuantity() * giohang.getProductEntity().getUnitPrice()}</span>
                            </c:otherwise>
                        </c:choose>
                    </td>

                    <td class="product-remove">
                        <a href="<c:url value='/cart/${giohang.getProductEntity().getId()}/remove'/>" class="remove" title="Remove this item"><i class="fa fa-trash-o"></i></a>
                    </td>
                </tr>
                ...
                <tr>
                    <td colspan="6" class="actions">
                        ...
                        <a class="btn btn-primary" href="<c:url value='/dathang'/>">Dat Hang</a>
        ...
        <div class="cart-collaterals">
            ...
            <div class="cart-totals pull-right">
                ...
                <div class="cart-totals-row"><span>Order Total:</span> <span id="totalPrice">$ ${total}</span></div>
            </div>
